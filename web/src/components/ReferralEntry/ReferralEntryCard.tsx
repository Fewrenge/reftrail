      ...
      <div className={`bg-white border border-slate-200 rounded-2xl p-5 shadow-sm relative group transition-all ${isClickable ? 'hover:border-blue-300' : ''}`}>
        ...
        <div className="grid grid-cols-3 gap-4 mb-5 pb-4 border-b border-slate-100">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-700">{entry.patientHealthcardNumber || 'N/A'}</p>
          </div>

          {/* Phone */}
          <div>
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tight mb-1">Phone</p>
            <p className="text-sm font-medium text-slate-700">{entry.patientCell || 'N/A'}</p>
          </div>
        </div>
